"""Fortinet-FortiNDRCloud Integration for Cortex XSOAR - Unit Tests file
"""
...
def test_map_severity():
    from FortiNDRCloud import mapSeverity
...
def test_encode_args():
    from FortiNDRCloud import encodeArgsToURL
...
def test_test_module(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandTestModule
...
def test_get_sensors(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandGetSensors, encodeArgsToURL
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Sensors'
...
def test_get_device_list(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandGetDevices, encodeArgsToURL
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Devices'
...
def test_get_events_telemetry(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandGetEventsTelemetry
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Telemetry.Events'
...
def test_get_packetstats_telemetry(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandGetPacketstatsTelemetry
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Telemetry.Packetstats'
    assert response.outputs_key_field == 'data'
    assert response.outputs == mock_response[response.outputs_key_field]


def test_get_networkaccess_telemetry(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandGetNetworkTelemetry
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Telemetry.NetworkUsage'
    assert response.outputs_key_field == 'network_usage'
    assert response.outputs == mock_response[response.outputs_key_field]


def test_get_tasks(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandGetTasks
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Tasks'
...
def test_create_task(requests_mock):
    from FortiNDRCloud import Client, SensorClient, commandCreateTask
...
def test_get_detections(requests_mock):
    from FortiNDRCloud import Client, DetectionClient, commandGetDetections
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Detections'
...
def test_fetch_incidents(requests_mock):
    from FortiNDRCloud import Client, DetectionClient, commandFetchIncidents
    mock_response = util_load_json('test_data/detections_results.json')
    i = 1
...
def test_get_detection_rules(requests_mock):
    from FortiNDRCloud import Client, DetectionClient, commandGetDetectionRules
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Rules'
    assert response.outputs_key_field == 'rules'
    assert response.outputs == mock_response[response.outputs_key_field]


def test_get_detection_events(requests_mock):
    from FortiNDRCloud import Client, DetectionClient, commandGetDetectionRuleEvents
...
def test_create_detection_rule(requests_mock):
    from FortiNDRCloud import Client, DetectionClient, commandCreateDetectionRule
...
def test_resolve_detection(requests_mock):
    from FortiNDRCloud import Client, DetectionClient, commandResolveDetection
...
def test_get_entity_summary(requests_mock):
    from FortiNDRCloud import Client, EntityClient, commandGetEntitySummary
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Entity.Summary'
    assert response.outputs_key_field == 'summary'
    assert response.outputs == mock_response[response.outputs_key_field]


def test_get_entity_pdns(requests_mock):
    from FortiNDRCloud import Client, EntityClient, commandGetEntityPdns
    entityId = getRandomString(10)
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Entity.PDNS'
    assert response.outputs_key_field == 'passivedns'
    assert response.outputs == mock_response[response.outputs_key_field]


def test_get_entity_dhcp(requests_mock):
    from FortiNDRCloud import Client, EntityClient, commandGetEntityDhcp
    entityId = getRandomString(10)
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Entity.DHCP'
    assert response.outputs_key_field == 'dhcp'
    assert response.outputs == mock_response[response.outputs_key_field]


def test_get_entity_file(requests_mock):
    from FortiNDRCloud import Client, EntityClient, commandGetEntityFile
    entityId = getRandomString(10)
    ...
    assert response.outputs_prefix == 'FortiNDRCloud.Entity.File'
